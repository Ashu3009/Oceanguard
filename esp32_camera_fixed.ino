...
#define LED_PIN 33  // Built-in flash LED (if available)

void setup() {
  Serial.begin(115200);
  Serial.setDebugOutput(true);
  Serial.println();
  Serial.println("🌊 OceanGuard ESP32-CAM Starting...");
  ...
  // Test capture to ensure camera works
  Serial.println("🧪 Testing camera capture...");
  camera_fb_t * test_fb = esp_camera_fb_get();
  if (!test_fb) {
    Serial.println("❌ Test capture FAILED!");
    Serial.println("⚠️ Camera might be faulty or poorly connected");
    while(1) { delay(1000); } // Halt
  } else {
    Serial.printf("✅ Test OK! Image size: %d bytes\n", test_fb->len);
    esp_camera_fb_return(test_fb);
  }

  Serial.println("\n🚀 System Ready - Starting image capture loop...\n");
  delay(2000);
}
...
void loop() {
  ...
  Serial.printf("\n📸 Capture #%d - Taking photo...\n", ++capture_count);
  digitalWrite(LED_PIN, HIGH); // LED on during capture
  ...
  if (httpResponseCode > 0) {
    ...
    Serial.println("⚠️ Check if server is running on network (not localhost!)");
    Serial.println("⚠️ Server should be: python manage.py runserver 0.0.0.0:8000");
  }
